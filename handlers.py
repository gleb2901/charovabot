...
async def call_manager(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("Менеджер уже спешит к тебе 💬 Ожидай, она напишет тебе в личные сообщения 💕")

    user = update.message.from_user
    username = user.username or f"{user.first_name} {user.last_name or ''}".strip()

    await context.bot.send_message(
        chat_id=-1002652755228,  # 🔁 замени на свой chat_id группы
        text=f"📞 Пользователь @{username} нажал «Позвать менеджера»!"
    )
